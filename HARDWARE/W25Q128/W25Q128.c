...
void W25Q128_PageProgram(uint32_t Address, uint8_t *DataArray, uint16_t Count)
{
	...
	MySPI_SwapByte((Address >> 24) & 0xFF);		//交换发送地址24位（W25Q128新增的高位地址）
	MySPI_SwapByte((Address >> 16) & 0xFF);		//交换发送地址23~16位
	MySPI_SwapByte((Address >> 8) & 0xFF);		//交换发送地址15~8位
	MySPI_SwapByte(Address & 0xFF);				//交换发送地址7~0位
